  ...
  <div>
    ...
    >
      ...
      <template v-slot:limit="{row}">
        ...
        <div v-else>
          {{ row.limit }}张<span v-if="row.limit_type != 1">/{{ listLabel.limit_type.option[row.limit_type] }}</span>
        </div>
      </template>
      ...
      <template v-slot:get_time="{row}">
        <div v-if="row.get_start_time && row.get_end_time">
          {{ row.get_start_time.substr(0, 10) + '~' + row.get_end_time.substr(0, 10) }}
        </div>
      </template>

      <template v-slot:use_time="{row}">
        <div v-if="row.use_start_time && row.use_end_time">
          {{ row.use_start_time.substr(0, 10) + '~' + row.use_end_time.substr(0, 10) }}
        </div>
      </template>
  ...
  data() {
    return {
      columns: [
        ...
        { name: '金额', field: 'amount', width: 150, type: 'tag', after: '元' },
